...
(function () {
  ...
  const state = {
    ...
    /* 검색 페이지네이션 */
    currentApiQuery: "",
    currentPage: 1,        // 다음 로드 시 가져올 startIndex = currentPage * 40
    totalItems: 0,
    isLoadingMore: false,
  };
  ...
  const PAGE_SIZE    = 40;   // Google Books API 1회 최대
  ...
  function notify(message) {
    ...
    window.__appToastTimer = setTimeout(() => { toast.style.opacity = "0"; }, 2600);
  }
  ...
  async function fetchBookById(id) {
    ...
    return normalizeBookItem(await res.json());
  }
  ...
  /**
   * 검색 결과를 그리드에 렌더링.
   * @param {object[]} books      - 이번에 추가할 도서
   * @param {boolean}  append     - true: 이어붙임, false: 초기화
   */
  function renderSearchResults(books, append) {
    ...
    const totalStr   = state.totalItems > 0 ? ` (전체 약 ${state.totalItems.toLocaleString()}건)` : "";
    summary.textContent = `'${state.currentApiQuery}'에 대한 검색 결과 ${shownCount}건${totalStr}`;

    // "더 불러오기" 버튼: 다음 startIndex가 totalItems 미만이고 500 이하일 때
    const nextStartIndex = state.currentPage * PAGE_SIZE;
    const hasMore = state.totalItems > 0 && nextStartIndex < state.totalItems && nextStartIndex < 500;
    if (loadMoreWrapper) {
  ...
  /**
   * 새 검색 시작: 첫 페이지를 보여준 뒤 나머지 페이지를 조용히 추가합니다.
   */
  async function performSearch(rawQuery, type) {
    ...
    try {
      /* ── 1차: startIndex=0, maxResults=40 ── */
      const first = await searchBooksRaw(apiQuery, { maxResults: PAGE_SIZE, startIndex: 0 });
      state.totalItems  = first.totalItems;
      state.currentPage = 1;     // 다음 로드는 page 1부터

      first.items.forEach(b => (state.booksCache[b.id] = b));
      renderSearchResults(first.items, false);
      hideLoader();

      if (first.items.length === 0) return;  // 실제 결과 없음

      /* ── 2차·3차: startIndex=40, 80 을 순차 로드해서 조용히 추가 ── */
      for (let page = 1; page <= 2; page++) {
        await sleep(300);   // 레이트 리밋 방지 딜레이
        const startIndex = page * PAGE_SIZE;
        if (startIndex >= state.totalItems || startIndex >= 500) break;

        try {
          const extra = await searchBooksRaw(apiQuery, { maxResults: PAGE_SIZE, startIndex });
          const newBooks = extra.items.filter(b => {
          ...
          if (newBooks.length) {
            ...
            renderSearchResults(newBooks, true);
          }
        } catch (_) { /* 추가 페이지 실패는 무시 */ }
      }
    } catch (err) {
      console.error(err);
      hideLoader();
  ...
  /**
   * "더 불러오기" 버튼 — index.html: onclick="loadMoreResults()"
   */
  window.loadMoreResults = async function () {
    if (state.isLoadingMore) return;
    const nextStartIndex = state.currentPage * PAGE_SIZE;
    if (nextStartIndex >= state.totalItems || nextStartIndex >= 500) return;
    ...
    try {
      const apiQuery = buildApiQuery(state.currentApiQuery, "all");
      for (let i = 0; i < 2; i++) {   // 2페이지 추가 (최대 +80권)
        const startIndex = (state.currentPage + i) * PAGE_SIZE;
        if (startIndex >= state.totalItems || startIndex >= 500) break;
        await sleep(300);
        try {
          const { items } = await searchBooksRaw(apiQuery, { maxResults: PAGE_SIZE, startIndex });
          const newBooks = items.filter(b => {
            if (state.booksCache[b.id]) return false;
            state.booksCache[b.id] = b;
            return true;
          });
          if (newBooks.length) renderSearchResults(newBooks, true);
        } catch (_) { /* 개별 페이지 실패 무시 */ }
      }
      state.currentPage += 2;
    } finally {
      ...
      const loadMoreWrapper = document.getElementById("search-load-more-wrapper");
      if (loadMoreWrapper) {
        const nextIdx = state.currentPage * PAGE_SIZE;
        loadMoreWrapper.style.display =
          (nextIdx < state.totalItems && nextIdx < 500) ? "flex" : "none";
      }
    }
  };
  ...
  /* index.html 홈 화면: onsubmit="handleHomeSearch(event)" */
  ...
  async function fetchRecommendationCandidates(genres, keywords) {
    ...
    for (const q of queries) {
      await sleep(200);
      try {
        const { items } = await searchBooksRaw(q, { maxResults: 40, startIndex: 0, langRestrict: "ko" });
        items.forEach(b => {
          all[b.id] = b;
  ...
  window.handleRecommendation = async function (event) {
    ...
    try {
      ...
      grid.innerHTML = picks.map((b, i) => bookCardHTML(b, { aiReason: craftReason(b, style, keywords, i) })).join("");
  ...
  window.showBookDetail = async function (id) {
    ...
    try {
      ...
      if (!book) { book = await fetchBookById(id); state.booksCache[id] = book; }
  ...
  function saveAllReviews(data) { localStorage.setItem(REVIEWS_KEY, JSON.stringify(data)); }
  function getReviewsForBook(bookId) { return loadAllReviews()[bookId] || []; }
  function setReviewsForBook(bookId, reviews) {
    const all = loadAllReviews(); all[bookId] = reviews; saveAllReviews(all);
  }
  ...
  function renderReviewSection(bookId) {
    const reviews = getReviewsForBook(bookId).slice()
    ...
    list.innerHTML = reviews.length
      ? reviews.map(r => reviewCardHTML(r, bookId)).join("")
  ...
  window.submitReview = function (bookId) {
    ...
    if (state.editingReviewId) {
      ...
      reviews.push({ id: "rv_" + Date.now() + "_" + Math.random().toString(36).slice(2, 7), rating, comment, createdAt: Date.now() });
    }
